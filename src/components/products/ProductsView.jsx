    ...
    return (
        <div>
            <div className="product-box border-4 border-black p-4" key={product.product_id}>
                <Link to={`/product/${product.slug}`} aria-label={`View details for ${product.product}`}>
                                ...
                                {product.media.map((items, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className={`carousel-item ${idx === 0 ? "active" : ""}`}>
                                            <img src={mediaUrl} className="img-fluid w-100" alt={`Image of ${product.product}`} />
                                        </div>
                                    );
                                })}
                ...
                <div className="product-content px-0">
                    <div className="w-full flex items-start justify-between my-4 product-card">
                        <Link to={`/product/${product.slug}`} aria-label={`More about ${product.product}`}>
